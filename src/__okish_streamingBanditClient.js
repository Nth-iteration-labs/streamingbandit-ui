...
export default (apiUrl, httpClient = jsonApiHttpClient) => {
    ...
    const convertHTTPResponseToREST = (response, type, resource, params) => {
        ...
        case GET_MANY_REFERENCE:
        case GET_LIST:
            var jsonData = json.map(function (dic) {
                    ...
                    Object.keys(dic.relationships).forEach(function(key){
                        ...
                        if (dic.relationships[key].data){
                            ...
                            httpClient(link).then(function (response) {
                                interDic[key] = {'data': response.json, 'count': response.json.length};
                                interDic['count'] = response.json.length;
                            });
                        }
                    })
                }
            ...
            return { data: jsonData, total: response.json.length };
        case GET_MANY:
