  ...
  const convertHTTPResponseToREST = (response, type, resource, params) => {
    ...
    switch (type) {
      ...
      case GET_MANY_REFERENCE:
        ...
        const totalLength = response.json.length;

        if (params.sort.order === "DESC") {
          response.json = _.reverse(
            _.sortBy(response.json, [params.sort.field])
          );
        } else {
        ...
        response.json = response.json.slice(offset, offset + perPage);

        return { data: response.json, total: totalLength };
      case UPDATE:
      case CREATE:
